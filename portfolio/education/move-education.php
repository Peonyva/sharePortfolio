...
try {
    ...
    $stmt = $conn->prepare("SELECT id FROM education WHERE userID = :userID AND sortOrder = :newSort LIMIT 1");
    $stmt->execute([
        ':userID' => $userID,
        ':newSort' => $newSort
    ]);
    ...
    $stmt = $conn->prepare("UPDATE education SET sortOrder = :currentSort WHERE id = :swapId AND userID = :userID");
    $stmt->execute([
    ...
    $stmt = $conn->prepare("UPDATE education SET sortOrder = :newSort WHERE id = :currentId AND userID = :userID");
    $stmt->execute([
        ':newSort' => $newSort,
        ':currentId' => $currentId,
        ':userID' => $userID
